...
public class DropDownMenu extends LinearLayout {
    ...
    public void setTabText(String text) {
        if (current_tab_position != -1) {
            Log.e(TAG, "setTabText: " + current_tab_position );
    ...
    public void setTabText(String text, int position) {
        int i = position * 2;
        if (i > taText.size() * 2) {
            throw new RuntimeException("不能找到该控件");
        }
        if (position > taText.size())
        Log.e("setTabText: ", current_tab_position + "");
        ((TextView) tabMenuView.getChildAt(i )).setText(text);
    }
